...
TANK_PARAM_BOUNDS: Dict[str, Tuple[float, float]] = {
    # T0: 表层水箱 (快速地表径流)
    't0_is': (0.0, 100.0),
    't0_boc': (0.01, 0.5),     # 拓宽下界，允许弱透水
    't0_soc_uo': (0.1, 0.8),   # 快速径流系数可以更大
    't0_soc_lo': (0.05, 0.5),
    't0_soh_uo': (30.0, 120.0), # 允许与下孔搜索空间重叠
    't0_soh_lo': (0.0, 80.0),

    # T1: 中间层水箱 (壤中流)
    't1_is': (0.0, 100.0),
    't1_boc': (0.005, 0.3),    # 下渗率应小于上层
    't1_soc': (0.01, 0.3),     # 出流率应小于上层
    't1_soh': (0.0, 100.0),

    # T2: 深层水箱 (浅层地下水/慢速壤中流)
    't2_is': (0.0, 300.0),     # 放宽初始水量上限
    't2_boc': (0.001, 0.1),
    't2_soc': (0.005, 0.1),
    't2_soh': (0.0, 50.0),     # 深层孔高通常较低

    # T3: 基流地下水箱
    't3_is': (0.0, 1000.0),    # 地下水初始储量可以很大
    't3_soc': (0.0001, 0.05),  # 极慢的出流系数 (关键修正)
}
