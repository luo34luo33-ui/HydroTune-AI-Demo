...
from algos.de import optimize_de
from models.hbv import run_hbv_model, HBV_PARAM_BOUNDS
from calibration_params import DE_PARAMS
...
def run_calibration():
    print("HBV+DE")
    events = load_events()
    pnames = list(HBV_PARAM_BOUNDS.keys()) + ['k_routing', 'x_routing']
    bounds = list(HBV_PARAM_BOUNDS.values()) + [MUSKINGUM_BOUNDS['k_routing'], MUSKINGUM_BOUNDS['x_routing']]
    ...
    def obj(p):
        ...
        try:
            ns = []
            for e in events:
                s = run_hbv_model(e['precip'], e['evap'], mp, CATCHMENT_AREA)
    ...
    bx, bf = optimize_de(obj, bounds, MAX_ITERATIONS, len(pnames), DE_PARAMS)
    bp = {k:v for k,v in zip(pnames, bx)}
    print(f"NSE:{-bf:.4f}")
    pd.DataFrame([bp]).to_csv(os.path.join(OUTPUT_PARAMS, "hbv_de_params.csv"), index=False)
    
    for e in events:
        s = run_hbv_model(e['precip'], e['evap'], bp, CATCHMENT_AREA)
        ...
        ax_flow.set_title(f"HBV-DE {e['name']}")
        ax_flow.legend(loc='upper left')
        ax_flow.grid(True, alpha=0.3)
        ax_flow.set_xlabel('Time Step (h)')
        ax_flow.set_ylabel('Flow (m³/s)')
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_PLOTS, "hbv", f"hbv_de_{e['name']}.png"), dpi=150, bbox_inches='tight')
        plt.close()
        pd.DataFrame({'obs':obs_plot, 'sim':sim_plot}).to_csv(os.path.join(OUTPUT_DATA, "hbv", f"hbv_de_{e['name']}.csv"), index=False)
    print("完成")
