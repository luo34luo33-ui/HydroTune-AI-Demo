...
def run_calibration():
    ...
    print(f"NSE:{-bf:.4f}")
    pd.DataFrame([bp]).to_csv(os.path.join(OUTPUT_PARAMS, "tank_ga_params.csv"), index=False)
    ...
    for e in events:
        ...
        ax_flow.set_title(f"Tank-GA {e['name']}")
        ax_flow.legend(loc='upper left')
        ax_flow.grid(True, alpha=0.3)
        ax_flow.set_xlabel('Time Step (h)')
        ax_flow.set_ylabel('Flow (m³/s)')
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_PLOTS, "tank", f"tank_ga_{e['name']}.png"), dpi=150, bbox_inches='tight')
        plt.close()
        pd.DataFrame({'obs':obs_plot, 'sim':sim_plot}).to_csv(os.path.join(OUTPUT_DATA, "tank", f"tank_ga_{e['name']}.csv"), index=False)
    print("完成")
